...
def validate_plan_does_not_resolve_conflicts(
    ...
    for mapping in plan.parameter_mapping:
        if label_hits_parameter_conflict(mapping.paper_param_name, conflicts):
            raise PaperPlanGenerationError(
                "parameter_conflict_mapping_stale",
                reason_code="parameter_conflict_mapping_stale",
            )
    ...
    if plan.m_script_skeleton and mscript_assigns_conflict_value(
        ...
        raise PaperPlanGenerationError(
            "parameter_conflict_mscript_stale",
            reason_code="parameter_conflict_mscript_stale",
        )
    ...
    for step in plan.build_steps:
        if text_contains_conflict_value(
            ...
            raise PaperPlanGenerationError(
                "parameter_conflict_build_step_text_stale",
                reason_code="parameter_conflict_build_step_text_stale",
            )
        for ref in step.parameter_refs:
            if label_hits_parameter_conflict(ref.paper_param_name, conflicts):
                raise PaperPlanGenerationError(
                    "parameter_conflict_build_step_ref_stale",
                    reason_code="parameter_conflict_build_step_ref_stale",
                )
        for hint in step.configuration_hints:
            if text_contains_conflict_value(
                ...
                raise PaperPlanGenerationError(
                    "parameter_conflict_configuration_hint_stale",
                    reason_code="parameter_conflict_configuration_hint_stale",
                )
...
def validate_tuning_does_not_resolve_conflicts(
    ...
    for direction in suggestion.parameter_directions:
        if label_hits_parameter_conflict(direction.param_name, conflicts):
            raise PaperPlanGenerationError(
                "parameter_conflict_tuning_ref_stale",
                reason_code="parameter_conflict_tuning_ref_stale",
            )
        if text_contains_conflict_value(
            ...
            raise PaperPlanGenerationError(
                "parameter_conflict_tuning_text_stale",
                reason_code="parameter_conflict_tuning_text_stale",
            )
