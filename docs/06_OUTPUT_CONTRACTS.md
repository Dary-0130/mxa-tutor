...
- `guidance_status="generated"` 必须携带 v2 `build_guidance.details`;TASK-538 v0.2 起,活性规则为至少一条可渲染 detail。无法盖章的文档 claim 必须降级为 `document_claim_unverified` / `user_confirmation_required` 且 `evidence=[]`,不得伪造 document badge;公共 JSON schema 字节不变。
