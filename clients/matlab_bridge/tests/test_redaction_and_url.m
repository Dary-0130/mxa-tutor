...
function testSubmitRunStateCoachingConfirmsScopeAndUsesExplainCapability(testCase)
contexts = strings(0, 1);
tokenCapabilities = strings(0, 1);
postedPayload = [];
app = mxa.bridge.MatlabBridgeApp( ...
    BaseUrl="http://localhost:8000", ...
    Visible="off", ...
...
submitted = app.submitRunStateCoaching( ...
    "user-alpha", ...
    "project-alpha", ...
    "11111111-1111-4111-8111-111111111111", ...
...
verifyTrue(testCase, submitted);
verifyEqual(testCase, contexts, "run_state_coaching");
verifyEqual(testCase, tokenCapabilities, "run_state:explain");
verifyEqual(testCase, string(postedPayload.protocol_version), "0.3-c1");
verifyEqual(testCase, string(postedPayload.coaching_consent_notice_version), ...
    "run_state_coaching_v1");
verifyEqual(testCase, double(postedPayload.previous_run_count), 0);
verifyTrue(testCase, contains(app.LastConfirmedSnapshot, "run_state_coaching_v1"));
verifyTrue(testCase, contains(app.LastConfirmedSnapshot, "22222222-2222-4222-8222-222222222222"));

    function confirmed = fakeConfirm(~, snapshot, context)
        verifyTrue(testCase, contains(string(snapshot), "previous_run_count"));
        contexts(end + 1, 1) = string(context); %#ok<AGROW>
        confirmed = true;
    end

    function token = fakeTokenProvider(~, ~, ~, capability)
        tokenCapabilities(end + 1, 1) = string(capability); %#ok<AGROW>
        token = "explain-token";
    end

    function result = fakePostCoaching(~, payload, timeoutSeconds, accessToken)
        postedPayload = payload;
        verifyEqual(testCase, timeoutSeconds, 60);
        verifyEqual(testCase, string(accessToken), "explain-token");
...
text = mxa.bridge.formatRunStateCoaching(result);

verifyTrue(testCase, contains(text, "运行状态陪调已生成"));
verifyTrue(testCase, contains(text, "outcome: coached"));
verifyTrue(testCase, contains(text, "摘要:"));
verifyTrue(testCase, contains(text, "建议方向:"));
verifyTrue(testCase, contains(text, "提示:"));
end
...
function result = makeRunStateCoaching(payload)
evidence = struct( ...
    "evidence_id", "e1", ...
...
reading = struct( ...
    "reading_id", "r1", ...
    "reading", "wall_clock_elapsed 说明本轮有可用指标。", ...
    "is_inference", true, ...
    "confidence", "medium", ...
    "evidence_ids", {{"e1"}});
direction = struct( ...
...
result = struct( ...
    ...
    "request_id", payload.request_id, ...
    "run_id", payload.run_id, ...
    "context_run_ids", {{payload.run_id}}, ...
    "status", "completed", ...
    "mode", "run_state_coaching", ...
    "outcome", "coached", ...
    "run_summary", "本轮状态为 completed,收敛状态为 not_applicable。", ...
    "signal_readings", reading, ...
    "primary_directions", direction, ...
    "cross_round_trend", [], ...
    "uncertainties", {{}}, ...
    "fallback_reason", [], ...
